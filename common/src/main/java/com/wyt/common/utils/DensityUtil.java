...
public class DensityUtil {
    ...
    private static final String HEIGHTORIENTATION = "height";
    private static final String WIDTHORIENTATION = "width";
     ...
     * 要在activity设置布局文件前
     */
    public static void setHeightOrientation(Activity activity) {
        setAppOrientation(activity, HEIGHTORIENTATION);
    }

    /**
     * 此方法用于在某一个Activity里面更改适配的方向
     */
    public static void setWidthOrientation(Activity activity) {
        setAppOrientation(activity, WIDTHORIENTATION);
    }
    ...
    private static void setAppOrientation(@Nullable Activity activity, String orientation) {
        ...
        //然后控件就直接用美工的dp就行了
        if (orientation == HEIGHTORIENTATION) {
            targetDensity = (appDisplayMetrics.heightPixels - barHeight) / 720f;
        } else {
