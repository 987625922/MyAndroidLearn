...
//                        LogUtil.d("下载数据","下载前的大小:"+downloadLength  + "  剩余大小:"+response.body().contentLength());
                        while ((len = is.read(buf)) != -1) {
                            ...
                            if (downCalls.containsKey(url)) {
                                ...
                                //信息打印
                                LogUtil.d("下载中", "进度：" + progress + " url=" + url);
                            }
                        }
                        // 下载完成
                        emitter.onComplete();
    ...
    public void getPercentBeforeDownload(final String url, Consumer<Integer> consumer) {
        Observable.create(new ObservableOnSubscribe<Integer>() {
            ...
            public void subscribe(ObservableEmitter<Integer> emitter) throws Exception {
                ...
                final long downloadLength = file.exists() ? file.length() : 0;
                final long contentLength = getContentLength(url);
                LogUtil.d("下载信息", "downloadLength=" + downloadLength + " contentLength=" + contentLength);
                if (contentLength == 0) {
                    emitter.onNext(0);
                } else {
    ...
    private long getContentLength(String url) {
        ...
        try {
            ...
            if (response != null && response.isSuccessful()) {
                ...
                LogUtil.d("检测长度", "下载前:" + contentLength);
                response.close();
